            ...
            for (let x = 0; x < newWidth; x++) {
                ...
                for (let c = 0; c < 3; c++) {
                    ...
                    if (textureStrength > 0.2) {
                        const noise = (Math.random() - 0.5) * textureStrength * 10;
                        value += noise;
                    }
        ...
        for (let y = 1; y < height - 1; y++) {
            for (let x = 1; x < width - 1; x++) {
                if (skinMask[y * width + x]) {
                    ...
                    // 肌の色調補正
                    data[idx] = Math.min(255, data[idx] * 1.05);     // R
                    data[idx + 1] = Math.min(255, data[idx + 1] * 1.02); // G
                }
            }
        }
    ...
    async enhance(blob, targetResolution = 'auto') {
        return new Promise((resolve) => {
            ...
            img.onload = async () => {
                ...
                // HDR風の処理
                finalCtx.filter = 'contrast(1.2) saturate(1.15) brightness(1.05)';
